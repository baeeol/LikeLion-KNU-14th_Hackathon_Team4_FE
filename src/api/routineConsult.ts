...
export async function getTroubleSolution(userId: number, trouble: string): Promise<TroubleSolutionResult> {
  const response = await fetch(`${API_BASE_URL}/user/${userId}/trouble_solution?trouble=${encodeURIComponent(trouble)}`);
...
export async function questionNewProduct(userId: number, productId: number): Promise<NewProductQuestionResult> {
  const response = await fetch(`${API_BASE_URL}/user/${userId}/question_new_product?productId=${productId}`);
