  ...
  return <SafeAreaView style={styles.safeArea}>
    ...
    <View style={styles.page}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.skinTypeInfoCard}><View style={styles.skinTypeInfoIcon}><Text style={styles.skinTypeInfoIconText}>◌</Text></View><View style={styles.skinTypeInfoCopy}><Text style={styles.skinTypeInfoLabel}>루틴 분석에 반영 중인 피부 타입</Text><Text style={styles.skinTypeInfoValue}>{user.skinType.type}</Text></View><Text style={styles.readOnlyLabel}>등록 정보</Text></View>
        <Text style={styles.skinHint}>온보딩에서 입력한 정보예요. 이 정보로 맞춤 루틴을 추천합니다.</Text>
...
function OwnedProductCard({product, onDelete}: {product: OwnedCareProduct; onDelete: () => void}) {return <View style={styles.ownedProduct}><View style={styles.ownedProductImage}><ProductIllustration category={product.category} /></View><View style={styles.ownedCopy}><Text style={styles.ownedCategory}>{product.category}</Text><Text style={styles.ownedName}>{product.brand} {product.name}</Text><Text style={styles.ownedFunction}>{product.usedInRoutine ? '현재 루틴에 사용 중인 제품' : '아직 루틴에 넣지 않은 제품'}</Text></View><View style={{alignItems: 'flex-end', gap: 6}}><Text style={[styles.ownedStatus, !product.usedInRoutine && styles.ownedInactiveStatus]}>{product.usedInRoutine ? '루틴 사용 중' : '보유 중'}</Text><Pressable onPress={onDelete} hitSlop={8}><Text style={{fontSize: 8, color: '#9A665D'}}>삭제</Text></Pressable></View></View>;}

function ProductIllustration({category: rawCategory}: {category: string}) {
  const category = rawCategory.toLowerCase();
  const isSunscreen = /sun_cream|sunscreen|선크림|자외선/.test(category);
  const isToner = /toner|skin|토너|스킨/.test(category);
  const isSerum = /serum|ampule|essence|세럼|앰플|에센스/.test(category);
  const isCream = !isSunscreen && /cream|크림|lotion|emulsion|로션|에멀전/.test(category);

  if (isToner) return <Image source={require('../assets/images/toner.png')} resizeMode="contain" style={styles.ownedTonerImage} />;
  if (isSerum) return <Image source={require('../assets/images/serum.png')} resizeMode="contain" style={styles.ownedSerumImage} />;
  if (isSunscreen) return <Image source={require('../assets/images/sunscreen.png')} resizeMode="contain" style={styles.ownedSunscreenImage} />;
  if (isCream) return <Image source={require('../assets/images/cream.png')} resizeMode="contain" style={styles.ownedCreamImage} />;
  return <Image source={require('../assets/images/cleanser.png')} resizeMode="contain" style={styles.ownedCleanserImage} />;
}
