      ...
      <View style={styles.welcomeRow}>
        <View>
          <View style={styles.welcomeTitleRow}>
            ...
            <Image
              source={require('../assets/images/routine-leaf.png')}
              resizeMode="contain"
              style={styles.welcomeLeafIcon}
            />
          </View>
          <Text style={styles.welcomeSub}>
            오늘의 피부에 가장 좋은 선택을 해주세요.
          </Text>
        </View>
      </View>
    </>
...
function RoutineSummary({
  ...
  const routine = (
    tab === 'morning' ? selectedRoutine.morning : selectedRoutine.evening
  ).map(toRoutineProduct);
  const routineDate =
    selectedDayIndex === todayIndex
      ? todayLabel
      : `${WEEKDAY_LABELS[selectedDayIndex]}요일 루틴`;
  return (
    <>
      <View style={styles.routineCard}>
        <View style={styles.routineHeader}>
          <View style={styles.titleRow}>
            <RoutineIcon />
            <Text style={styles.routineTitle}>내 루틴</Text>
            <Text style={styles.routineDate}>{routineDate}</Text>
          </View>
        ...
        <View style={styles.tipRow}>
          ...
          <Text style={styles.tipText}>
            {tab === 'morning' ? '아침' : '저녁'} 루틴 · 현재 등록된 제품
            순서예요.
          </Text>
        </View>
      </View>
...
function getRoutineForWeekday(routines: DailyRoutine[], dayIndex: number) {
  return routines[(dayIndex + 6) % 7] ?? { morning: [], evening: [] };
}
function toRoutineProduct(product: RoutineApiProduct): RoutineProduct {
  return {
...
function RoutineIcon() {
  return <Text style={styles.routineLeafIcon}>🌿</Text>;
}
function MoonIcon() {
  return (
    ...
    <View style={styles.routineItem}>
      <View style={styles.orderColumn}>
        <View style={styles.orderNumber}>
          <Text style={styles.orderNumberText}>{index}</Text>
        </View>
      ...
      <View style={styles.productShapeArea}>
        <ProductIllustration
          category={product.category}
          style={getRoutineImageStyle(product.category)}
        />
      </View>
...
function getRoutineImageStyle(category: string) {
  switch (getProductIllustrationKind(category)) {
    case 'toner':
      return styles.routineTonerImage;
    case 'serum':
      return styles.routineSerumImage;
    case 'cream':
      return styles.routineCreamImage;
    case 'sunscreen':
      return styles.routineSunscreenImage;
    default:
      return styles.routineCleanserImage;
  }
}
    ...
    <View style={styles.weeklyCard}>
      <View style={styles.weeklyHeader}>
        <View style={styles.weeklyTitleWrap}>
          ...
          />
          <Text style={styles.weeklyTitle}>주간 스케줄</Text>
        </View>
      </View>
      <View style={styles.scheduleRow}>
        {[1, 2, 3, 4, 5, 6, 0].map(dayIndex => {
          const routine = getRoutineForWeekday(routines, dayIndex);
          const label = getWeeklyFunctionLabel(routines, routine);
          return (
            <Pressable
              key={dayIndex}
              ...
              style={[
                styles.dayColumn,
                selectedDayIndex === dayIndex && styles.dayColumnSelected,
              ]}
            >
              <Text style={styles.dayText}>{WEEKDAY_LABELS[dayIndex]}</Text>
              <View style={[styles.scheduleTag, scheduleTagStyle(label)]}>
                <Text style={styles.scheduleTagText}>{label}</Text>
              </View>
            </Pressable>
          );
        })}
      ...
      <View style={styles.legendRow}>
        <Legend color="#69A174" label="기능성 루틴" />
        <Legend color="#CDD1CD" label="기본·휴식 루틴" />
      </View>
    </View>
  );
}

function getWeeklyFunctionLabel(
  routines: DailyRoutine[],
  routine: DailyRoutine,
) {
  const functionalProduct = [...routine.morning, ...routine.evening].find(
    product => {
      const functionName = getFunctionName(product.name);
      const usedDays = routines.filter(day =>
        [...day.morning, ...day.evening].some(
          item => item.id === product.id || item.name === product.name,
        ),
      ).length;
  ...
  if (!functionalProduct)
    return routine.morning.length + routine.evening.length ? '기본' : '휴식';
  return getFunctionName(functionalProduct.name);
}

function getFunctionName(name: string) {
  if (/AHA|BHA|각질/i.test(name)) return '각질';
  if (/레티놀/i.test(name)) return '레티놀';
  if (/비타민\s?C|나이아신/i.test(name)) return '미백·톤';
  if (/판테놀|시카|진정/i.test(name)) return '진정';
  return '';
}
function scheduleTagStyle(label: string) {
  if (label === '각질' || label === '레티놀') return styles.orangeTag;
  if (label === '진정') return styles.mintTag;
  if (label === '휴식') return styles.grayTag;
...
function ShortcutCard({
  ...
  return (
    <Pressable
      ...
      style={[
        styles.shortcut,
        variant === 'search' ? styles.searchShortcut : styles.sosShortcut,
      ]}
    >
...
const styles = StyleSheet.create({
  ...
  welcomeRow: { height: 80, justifyContent: 'center', paddingLeft: 12 },
  ...
  welcomeLeafIcon: { width: 26, height: 26, marginLeft: 4 },
  ...
  routineHeader: { paddingHorizontal: 13, paddingTop: 12, paddingBottom: 10 },
  ...
  routineLeafIcon: { fontSize: 19, marginRight: 7 },
  ...
  routineCleanserImage: { width: 42, height: 54 },
  routineTonerImage: { width: 36, height: 46 },
  routineSerumImage: { width: 40, height: 50 },
  ...
  calendarImage: { width: 23, height: 23, marginRight: 8 },
  ...
  scheduleTagText: { fontSize: 8, fontWeight: '800' },
  ...
  shortcutAiImage: { width: 25, height: 25 },
