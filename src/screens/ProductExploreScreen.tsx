...
import {addUserCareProduct, CareProduct, searchCareProducts} from '../api/careProduct';

const FILTERS = ['전체', '세정', '보습', '피부 장벽', '진정', '각질 케어', '트러블·피지', '미백·톤', '자외선 차단'] as const;
...
export function ProductExploreScreen({navigate, onPurchase}: {navigate: Navigate; onPurchase: (product: {id: number; category: string; brand: string; name: string; usedInRoutine: boolean}) => void}) {
  ...
  const [purchaseNotice, setPurchaseNotice] = useState<'success' | 'error' | null>(null);
  ...
  return <SafeAreaView style={styles.safeArea}>
    ...
    <View style={styles.page}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.searchBox}><Text style={styles.searchIcon}>⌕</Text><TextInput value={query} onChangeText={setQuery} placeholder="제품 종류, 브랜드, 제품명으로 검색" placeholderTextColor="#9AA39B" style={styles.searchInput} /></View>
...
function ProductCard({product, selected, onPress}: {product: CareProduct; selected: boolean; onPress: () => void}) {
  ...
  return <Pressable onPress={onPress} style={[styles.productCard, productLayout.card, selected && styles.productCardSelected]}><View style={styles.productImage}><ProductIllustration product={product} functions={functions} /></View><Text style={styles.brand}>{product.brand}</Text><Text style={styles.productName}>{product.name}</Text><Text style={styles.price}>{formatPrice(product.price)}</Text><View style={styles.tag}><Text style={styles.tagText}>{functions[0] ?? product.category}</Text></View></Pressable>;
}

function ProductIllustration({product, functions}: {product: CareProduct; functions: string[]}) {
  const category = product.category.toLowerCase();
  const isSunscreen = /sun_cream|sunscreen|선크림|자외선/.test(category);
  const isCleanser = /cleanser|클렌저|클렌징|폼|oil|오일/.test(category);
  ...
  if (isCleanser) return <Image source={require('../assets/images/cleanser.png')} resizeMode="contain" style={styles.cleanserIllustration} />;
  if (isToner) return <Image source={require('../assets/images/toner.png')} resizeMode="contain" style={styles.tonerIllustration} />;
  if (isSerum) return <Image source={require('../assets/images/serum.png')} resizeMode="contain" style={styles.serumIllustration} />;
  if (isSunscreen) return <Image source={require('../assets/images/sunscreen.png')} resizeMode="contain" style={styles.sunscreenIllustration} />;
  if (isCream) return <Image source={require('../assets/images/cream.png')} resizeMode="contain" style={styles.creamIllustration} />;

  return <Image source={require('../assets/images/cleanser.png')} resizeMode="contain" style={styles.cleanserIllustration} />;
}
